-- LOAD ALL 68 SEENAF_CTF CHALLENGES
-- Run this in Supabase SQL Editor to load the complete challenge set

-- Clear existing challenges
DELETE FROM challenges;

-- Insert all 68 challenges
INSERT INTO challenges (title, description, category, difficulty, points, flag, hints, author, instance_url, is_active) VALUES

-- Web Security Challenges (8)
...
-- Reverse Engineering Challenges (8)
...
-- Binary Exploitation (Pwn) Challenges (5)
...
('Digital Footprint Analysis', 'Analyze the digital footprint of this target to find sensitive information.', 'OSINT', 'hard', 350, 'SEENAF{digital_footprint_traced}', ARRAY['Search across multiple platforms', 'Look for data breaches', 'Check archived web pages'], 'OSINT Team', 'https://osint-challenges.seenaf-ctf.com/footprint', true),

-- Steganography Challenges (6)
...
('Polyglot Steganography', 'This file appears to be multiple formats at once. Find the hidden flag.', 'Stego', 'insane', 500, 'SEENAF{polyglot_file_decoded}', ARRAY['Check file signatures', 'Try opening as different file types', 'Look for embedded data'], 'Stego Team', 'https://stego-challenges.seenaf-ctf.com/polyglot', true),
...
('Quantum Computing', 'Solve this quantum computing problem to get the flag.', 'Misc', 'insane', 600, 'SEENAF{quantum_computing_mastered}', ARRAY['Understand quantum gates', 'Use quantum simulators', 'Apply quantum algorithms'], 'Misc Team', 'https://misc-challenges.seenaf-ctf.com/quantum', true),

-- Expert Level Challenges (10)
...
('Hypervisor Escape', 'Escape from this hypervisor to access the host system.', 'Reverse', 'insane', 700, 'SEENAF{hypervisor_escaped}', ARRAY['Research hypervisor vulnerabilities', 'Look for memory corruption bugs', 'Craft exploit payloads'], 'Reverse Team', 'https://reverse-challenges.seenaf-ctf.com/hypervisor', true),

('Kernel Exploitation', 'Exploit this kernel vulnerability to gain root privileges.', 'Pwn', 'insane', 700, 'SEENAF{kernel_pwned}', ARRAY['Understand kernel internals', 'Look for privilege escalation bugs', 'Bypass kernel protections'], 'Pwn Team', 'https://pwn-challenges.seenaf-ctf.com/kernel', true),

('Deep Web Investigation', 'Navigate the deep web to find this hidden service and extract the flag.', 'OSINT', 'insane', 600, 'SEENAF{deep_web_explored}', ARRAY['Use Tor browser safely', 'Search hidden service directories', 'Follow the breadcrumbs'], 'OSINT Team', 'https://osint-challenges.seenaf-ctf.com/deepweb', true),
...
('Advanced Persistent Steganography', 'This steganography technique survives multiple transformations. Find the flag.', 'Stego', 'insane', 600, 'SEENAF{persistent_stego_master}', ARRAY['Try robust steganography techniques', 'Test against compression and resizing', 'Look for spread spectrum methods'], 'Stego Team', 'https://stego-challenges.seenaf-ctf.com/persistent', true),

('Meta-Challenge', 'This challenge requires solving multiple other challenges to unlock the final flag.', 'Misc', 'insane', 1000, 'SEENAF{meta_challenge_ultimate_master}', ARRAY['Solve prerequisite challenges first', 'Look for patterns across challenges', 'Combine solutions from different categories'], 'SEENAF Team', 'https://misc-challenges.seenaf-ctf.com/meta', true);

-- Verify the load
SELECT 'All 68 challenges loaded successfully!' as status;
...
SELECT 'Challenge distribution:' as info;
SELECT 
    difficulty,
    COUNT(*) as count,
